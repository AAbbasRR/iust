...
class UserRegisterApiTestCase(TestUserSetUp):
    ...
    def register_api_test(self):
        ...
        redis_management = Redis(
            response_json["email"], f"{RedisKeys.activate_account}_otp_code"
        )
    ...
    def active_account_api_test(self):
        ...
        # expire otp_code key
        redis_management = Redis(
            self.success_register["email"], f"{RedisKeys.activate_account}_otp_code"
        )
        redis_management.delete()
        ...
        response = self.client.post(
            self.active_account_api,
            {"email": self.success_register["email"], "otp_code": self.otp_code},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.success_user_obj = UserModel.objects.find_by_email(
            self.success_register["email"]
        )
    ...
    def resend_activation_code_api_test(self):
        ...
        # check invalid email address
        response = self.client.post(
            self.resend_activation_otp_api, self.invalid_email_address
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response_json = response.json()
        response_keys = list(response_json.keys())
        ...
        # check email not found
        response = self.client.post(
            self.resend_activation_otp_api, self.not_found_account
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        response_json = response.json()
        response_keys = list(response_json.keys())
        ...
        # user is active
        response = self.client.post(
            self.resend_activation_otp_api, self.success_register
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response_json = response.json()
        ...
        response = self.client.post(
            self.resend_activation_otp_api, self.user_not_activated
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        redis_management = Redis(
            self.user_not_activated["email"], f"{RedisKeys.activate_account}_otp_code"
        )
