...
class UserRegisterApiTestCase(TestUserSetUp):
    ...
    def forget_password_api_test(self):
        ...
        response = self.client.post(self.forget_password_api, self.invalid_email_address)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response_json = response.json()
        response_keys = list(response_json.keys())
        self.assertTrue('email' in response_keys)
        ...
        response = self.client.post(self.forget_password_api, self.success_forget_password)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        redis_management = Redis(self.success_forget_password['email'], f'{RedisKeys.forget_password}_otp_code')
    ...
    def validate_forget_password_api_test(self):
        ...
        # check invalid email address
        response = self.client.post(self.validate_forget_password_api, self.invalid_email_address)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response_json = response.json()
        response_keys = list(response_json.keys())
        self.assertTrue('email' in response_keys)
        ...
        # check email not found
        response = self.client.post(self.validate_forget_password_api, self.not_found_account)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        response_json = response.json()
        response_keys = list(response_json.keys())
        self.assertTrue('detail' in response_keys)
        ...
        response = self.client.post(self.validate_forget_password_api, self.success_forget_password)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response_json = response.json()
    ...
    def complete_forget_password_api_test(self):
        ...
        # check invalid email address
        response = self.client.post(self.complete_forget_password_api, self.invalid_email_address)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response_json = response.json()
        response_keys = list(response_json.keys())
        self.assertTrue('email' in response_keys)
        ...
        # check email not found
        response = self.client.post(self.complete_forget_password_api, self.not_found_account)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        response_json = response.json()
        response_keys = list(response_json.keys())
        self.assertTrue('detail' in response_keys)
        ...
        response = self.client.post(self.complete_forget_password_api, self.invalid_password)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response_json = response.json()
        response_keys = list(response_json.keys())
        self.assertTrue('password' in response_keys)
        ...
        response = self.client.post(self.complete_forget_password_api, self.invalid_passwords_match)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response_json = response.json()
        response_keys = list(response_json.keys())
        self.assertTrue('detail' in response_keys)
        ...
        response = self.client.post(self.complete_forget_password_api, self.success_forget_password)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        redis_management = Redis(self.success_forget_password['email'], RedisKeys.forget_password)
        self.assertFalse(redis_management.exists())
        self.assertLessEqual(redis_management.get_expire(), -1)
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response_json = response.json()
        self.assertEqual(response_json['detail'], BaseErrors.user_dont_have_forget_password_permission)
        redis_management = Redis(self.success_forget_password['email'], RedisKeys.forget_password)
        self.assertFalse(redis_management.exists())
        self.assertLessEqual(redis_management.get_expire(), -1)
